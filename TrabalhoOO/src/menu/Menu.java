...
public class Menu {
	...
	static Conta buscarContaPessoaFisica(String identificador) {
		...
		if(contas.size() > 0) {
			for(ContaPessoaFisica cc : contas) {
				if(cc.getCpf().equals(identificador) || cc.getRg().equals(identificador)) {
					conta = cc;
				}
			}
		}
	...
	static void transferir() {
		System.out.println("Identificar da conta do remente (rg, cpf): ");
		String identificador = input.next();
		
		Conta conta = buscarContaPessoaFisica(identificador);
